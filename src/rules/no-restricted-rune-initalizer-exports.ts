...
export const noExportedRuneInitializersRule: Rule.RuleModule = {
  meta: {
    ...
    docs: {
      description:
        "Disallow the export of variables or values initialized with a Svelte rune CallExpression.",
    ...
    return {
      VariableDeclaration(node) {
        for (const declarator of node.declarations) {
          if (
            ...
            RUNE_NAMES.includes(
              declarator.init.callee.name as (typeof RUNE_NAMES)[number]
            )
          ) {
            runeInitializers.set(
              declarator.id.name,
      ...
      ExportDefaultDeclaration(node) {
        if (
          ...
          RUNE_NAMES.includes(
            node.declaration.callee.name as (typeof RUNE_NAMES)[number]
          )
        ) {
          context.report({
          ...
          runeInitializers.has(node.declaration.name)
        ) {
          context.report({
            node: node.declaration,
            messageId: "noExportedRunes",
