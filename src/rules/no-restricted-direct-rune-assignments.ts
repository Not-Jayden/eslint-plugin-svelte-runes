...
export const noRestrictedDirectRuneAssignments: Rule.RuleModule = {
  ...
  create(context) {
    if (
      !SVELTE_FILE_EXTENSIONS.some((extension) =>
        context.filename.endsWith(extension)
      )
    ) {
      return {};
    }
    ...
    return {
      Property(node) {
        function getIdentifierOfInterest(
          node: Extract<Rule.Node, { type: "Property" }>
        ) {
        ...
        if (
          !RUNE_NAMES.includes(
            identifierOfInterest.name as (typeof RUNE_NAMES)[number]
          )
        ) {
          return;
        }
      ...
      VariableDeclarator(node) {
        ...
        if (
          !RUNE_NAMES.includes(node.init.name as (typeof RUNE_NAMES)[number])
        ) {
          return;
        }
